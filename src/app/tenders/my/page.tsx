      ...
      <div className="min-h-screen bg-gray-50 dark:bg-[#0a1228] py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="animate-pulse">
            <div className="h-12 bg-gray-200 dark:bg-white/10 rounded-lg w-64 mb-4" />
            <div className="h-6 bg-gray-200 dark:bg-white/10 rounded-lg w-96 mb-8" />
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="bg-white dark:bg-white/5 rounded-xl p-5 space-y-3">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8 pb-4 border-b border-gray-200 dark:border-white/10">
            <div className="flex flex-wrap justify-between items-center gap-4">
              <div>
                <h1 className="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-cyan-300 bg-clip-text text-transparent">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="px-4 py-2 text-sm font-medium rounded-xl bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-white/80 hover:bg-gray-200 dark:hover:bg-white/20 transition flex items-center gap-2"
                >
                ...
                <button
                  onClick={() => router.back()}
                  className="px-4 py-2 text-sm font-medium rounded-xl bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-white/80 hover:bg-gray-200 dark:hover:bg-white/20 transition"
                >
                  ← Back
                </button>
              </div>
            ...
            {hasTenders && (
              <div className="mt-6 flex gap-4 text-sm text-gray-600 dark:text-white/60">
          ...
          {/* Search bar */}
          <div className="bg-white/60 dark:bg-white/5 backdrop-blur-md rounded-2xl border border-gray-200 dark:border-cyan-500/30 p-4 mb-8 shadow-lg dark:shadow-none">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-3 py-2.5 bg-white dark:bg-white/5 border border-gray-300 dark:border-cyan-500/30 rounded-xl text-sm text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-white/40 focus:border-blue-500 dark:focus:border-cyan-400 focus:ring-2 focus:ring-blue-500/20 dark:focus:ring-cyan-400/20 transition"
                />
              </div>
              {search && (
                <button
                  onClick={() => setSearch("")}
                  className="px-5 py-2.5 bg-gray-200 hover:bg-gray-300 dark:bg-white/10 dark:hover:bg-white/20 text-gray-700 dark:text-white rounded-xl font-medium transition"
                >
                  Clear
                </button>
              )}
          ...
          {showEmptyState && (
            <div className="bg-white/70 dark:bg-white/5 backdrop-blur-sm rounded-3xl border border-gray-200 dark:border-cyan-500/30 p-12 text-center shadow-xl">
              <div className="max-w-md mx-auto">
                <div className="w-24 h-24 mx-auto mb-6 text-gray-400 dark:text-white/30">
                  <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                </div>
                <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">No submissions yet</h2>
                <p className="text-gray-600 dark:text-white/60 mt-2">
          ...
          {/* No search results */}
          {showNoSearchResults && (
            <div className="bg-white/70 dark:bg-white/5 backdrop-blur-sm rounded-2xl border border-gray-200 dark:border-cyan-500/30 p-8 text-center">
              <p className="text-gray-600 dark:text-white/60">No tenders match your search.</p>
              <button onClick={() => setSearch("")} className="mt-3 text-blue-600 dark:text-cyan-400 hover:underline">
                Clear search
              </button>
            </div>
          )}
              ...
              {filteredTenders.map((tender) => {
                ...
                return (
                  <Link
                    ...
                    className={`${cardWidthClass} group block relative bg-white dark:bg-white/5 backdrop-blur-sm rounded-2xl border border-gray-200 dark:border-cyan-500/30 overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 hover:border-cyan-400 dark:hover:border-cyan-400`}
                  ...
                  >
                    <div className="p-5">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs mb-4">
                        <div>
                          <span className="text-gray-500 dark:text-white/40">Bid Deadline</span>
                          <p className="font-medium text-gray-800 dark:text-white/80">{formatDate(tender.closing_date)}</p>
                        </div>
                        <div>
                          <span className="text-gray-500 dark:text-white/40">Renovation</span>
                      ...
                      {tender.closing_date && (
                        <div className={`text-xs font-medium mb-3 p-2 rounded-lg ${
                          isDeadlineSoon 
                            ...
                            : "bg-gray-50 dark:bg-white/5 text-gray-600 dark:text-white/60"
                        }`}>
                          {daysLeft === 0 ? "Deadline passed" : daysLeft === 1 ? "Closes tomorrow" : `Closes in ${daysLeft} days`}
                        </div>
                      )}

                      <div className="mt-3 pt-2 border-t border-gray-100 dark:border-white/10 flex justify-between items-center text-xs">
                        <span className="text-gray-500 dark:text-white/40">Your BQ status:</span>
                        <span className={`font-medium ${submissionColor}`}>{submissionDisplay}</span>
                      </div>

                      <div className="mt-3 w-full flex items-center justify-center gap-2 px-4 py-2 bg-gray-100 dark:bg-white/10 group-hover:bg-gray-200 dark:group-hover:bg-white/20 text-gray-700 dark:text-white rounded-xl text-sm font-medium transition">
                        View Details
